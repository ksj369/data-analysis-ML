...
# ╔═╡ 9633e194-df98-4916-85f3-48d8f4059e56
...
# ╔═╡ 1e967733-cfc1-4319-9318-f8775b2e58c8


# ╔═╡ 99552d54-d5e3-4aeb-b50d-04156411fdf6
...
function epoch!(reg::MiniBatchRegressor,
	...
	for i in 1:Int((length(Y)/reg.b))
		id=rand[(i-1)*reg.b+1:i*reg.b]
		w=w-η*sum(w.*X[id].-Y[id].*X[id])/reg.b
	end
	reg.w=w
...
let
	if __run_q3b_experiments
		...
		mean(results3b["Mean"].test_error)
	end
end
